...
class DataLoader:
    def __init__(self, input_path: str = None):
        ...
        self.column_maps = {
            ...
            'estabelecimento': {0: 'cnpj_basico', 1: 'cnpj_ordem', 2: 'cnpj_dv', 3: 'identificador_matriz_filial',
                                ...
                                26: 'fax', 27: 'correio_eletronico', 28: 'situacao_especial',
        ...
        self.schemas = {
            'empresa': {'cnpj_basico': pl.Utf8, 'razao_social': pl.Utf8,
                        'natureza_juridica': pl.Int32, 'qualificacao_responsavel': pl.Int32,
                        'capital_social': pl.Float64, 'porte_empresa': pl.Int32,
                        'ente_federativo_responsavel': pl.Utf8},
            'estabelecimento': {'cnpj_basico': pl.Utf8, 'cnpj_ordem': pl.Utf8, 'cnpj_dv': pl.Utf8,
                                ...
                                'pais': pl.Int32, 'data_inicio_atividade': pl.Int32,
                                'cnae_fiscal_principal': pl.Int32, 'cnae_fiscal_secundaria': pl.Utf8,
                                'tipo_logradouro': pl.Utf8, 'logradouro': pl.Utf8, 'numero': pl.Utf8,
                                'complemento': pl.Utf8, 'bairro': pl.Utf8, 'cep': pl.Utf8, 'uf': pl.Utf8,
                                'municipio': pl.Int32, 'ddd_1': pl.Utf8, 'telefone_1': pl.Utf8,
                                'ddd_2': pl.Utf8, 'telefone_2': pl.Utf8, 'ddd_fax': pl.Utf8, 'fax': pl.Utf8,
                                'correio_eletronico': pl.Utf8, 'situacao_especial': pl.Utf8,
                                'data_situacao_especial': pl.Int32},
            'socios': {'cnpj_basico': pl.Utf8, 'identificador_socio': pl.Int32,
                       'nome_socio_razao_social': pl.Utf8, 'cpf_cnpj_socio': pl.Utf8,
                       'qualificacao_socio': pl.Int32, 'data_entrada_sociedade': pl.Int32,
            ...
            'simples': {'cnpj_basico': pl.Utf8, 'opcao_pelo_simples': pl.Utf8,
                        'data_opcao_simples': pl.Int32, 'data_exclusao_simples': pl.Int32,
                        'opcao_mei': pl.Utf8, 'data_opcao_mei': pl.Int32,
    ...
    def find_files_by_type(self, file_type: str) -> List[str]:
        ...
        input_files = [
            f for f in glob.glob(os.path.join(self.input_path, '*'))
            if re.search(pattern, os.path.basename(f), re.IGNORECASE)
        ]
        temp_files = [
            f for f in glob.glob(os.path.join(self.temp_path, '*'))
            if re.search(pattern, os.path.basename(f), re.IGNORECASE)
        ]
    ...
    def _read_file_utf8(self, file_path: str) -> io.StringIO:
        """Detecta charset do arquivo e converte para UTF-8, retornando um StringIO."""
        with open(file_path, 'rb') as f:
            rawdata = f.read(200_000)
            result = chardet.detect(rawdata)
            charset = result['encoding'] or 'utf-8'
            confidence = result['confidence']
            logger.info(f"Charset detectado: {charset} (confiança: {confidence:.2f})")
    ...
    def process_file(self, file_path: str, file_type: str) -> bool:
        try:
            ...
            csv_buffer = self._read_file_utf8(file_path)
            ...
            with db_manager.get_connection() as conn:
                with conn.cursor() as cursor:
                    for i, chunk_df in enumerate(df_lazy.collect(streaming=True)):
                        ...
                        chunk_df.write_csv(buffer_out, separator='\t', include_header=False)
                        buffer_out.seek(0)
